    ...
    public partial class Proveedores : Form
    {
        ...
        public void CargarCamposClientes()
        {
            ...
            else
            {
                ...
                tMail.Text = dataSet.Tables["proveedores"].Rows[0]["EMailProveedor"].ToString();
                tComentarios.Text = dataSet.Tables["proveedores"].Rows[0]["ComentariosProveedor"].ToString();
            }
        }
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            if (routines.SqlAccion("UPDATE Proveedores SET Estado =" + (checkBox1.Checked ? 1 : 0) + ", ApellidoProveedor='" + tApellido.Text.Trim().ToUpper().Replace("'", "´") + "' , NombreProveedor='" + tNombre.Text.Trim().ToUpper().Replace("'", "´") + "', DocumentoProveedor=" + Conversion.Val(tDNI.Text.Trim().Replace(".", "").Replace(" ", "").Replace(",", "")) + ", CuitProveedor =" + Conversion.Val(tCUIT.Text.Trim().Replace(".", "").Replace(" ", "").Replace(",", "")) + ", DomicilioProveedor='" + tDireccion.Text.Trim().ToUpper().Replace("'", "´") + "', ProvinciaProveedor='" + tProvincia.Text.Trim().ToUpper().Replace("'", "´") + "', LocalidadProveedor='" + tLocalidad.Text.Trim().ToUpper().Replace("'", "´") + "', PostalProveedor='" + tCP.Text.Trim().ToUpper().Replace("'", "´") + "', EMailProveedor='" + tMail.Text.Trim().ToUpper().Replace("'", "´") + "', FechaNacimientoProveedor=" + routines.FechaSql(dateTimePicker1.Value) + ", TelefonoProveedor=" + Conversion.Val(tTelefono.Text.Trim().Replace(".", "").Replace(" ", "").Replace(",", "")) + ", ComentariosProveedor='" + tComentarios.Text.Trim().ToUpper().Replace("'", "´") + "' WHERE NProveedor=" + routines.Vnum(lIdProveedor.Text)) == true)
            {
        ...
        private void cerrarSesiónToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Form1 F = new Form1();
            DialogResult opcion;
            opcion = MessageBox.Show("¿Desea cerrar la sesión?", "¡Atención!", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
